...
if ( strlen( $link ) > 0 && 'none' !== $link ) {
	...
	if ( 'custom' === $link && ! empty( $url ) ) {
		...
		*/
		...
		$image_block .= '<a href="#" title="{{ post_title }}" onclick="event.preventDefault();event.stopPropagation();return false;" ' . $this->prettyphoto_rel . ' data-vc-gitem-zone="prettyphotoLink" class="vc_gitem-link prettyphoto vc-zone-link vc-prettyphoto-link"></a>';
	}
